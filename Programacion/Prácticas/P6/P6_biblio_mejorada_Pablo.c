...
void añadirLibro(LIBRO * catalogo, int cant_añadir){
	...
	LIBRO * catalogoNuevo = (LIBRO *)realloc(catalogo, tam * sizeof(LIBRO)); //Creo un nuevo ctalogo en el que guardo el catalogo y aumento su memoria
	...
	for (int i = 0; i < tam; i++){  //Imprimo el catalogo más los añadidos
...
int main(int argc, char * argv[]){
	...
	LIBRO * catalogo;
	catalogo = (LIBRO*)malloc(sizeof(LIBRO) * MAX); //Reservo memoria 
	if (catalogo == NULL){
		printf("ERROR");
	}
    ...
    if (argc > 1){
    	...
    	else if (strcmp(argv[1], "mostrarLibro") == 0 && atoi(argv[2]) <= 40 && atoi(argv[2]) >= 1){  //atoi = ASCII to Int
